...
class Careers_JsonController extends Zend_Controller_Action
{
   ...
   public function indexAction()
	{
		...
		//SELECT * FROM `hrm_employees` LEFT JOIN hrm_agreements ON hrm_employees.employee_id=hrm_agreements.employee_id WHERE DATE(hrm_agreement.effective_date) <= NOW();
		
		if ($fields == "firstname") {
		
			$sql_count = "SELECT * FROM `hrm_employees` LEFT JOIN hrm_agreements ON hrm_employees.employee_id=hrm_agreements.employee_id WHERE DATE(hrm_agreements.start_date) <= NOW() AND DATE(hrm_agreements.effective_date) >= NOW() - INTERVAL 1 DAY"
			...
			$sql = 'SELECT * FROM `hrm_employees` '
				...
				$sql_count = "SELECT * FROM `hrm_employees` LEFT JOIN hrm_agreements ON hrm_employees.employee_id=hrm_agreements.employee_id WHERE DATE(hrm_agreements.start_date) <= NOW() AND DATE(hrm_agreements.effective_date) >= NOW() - INTERVAL 1 DAY"
						." AND hrm_employees.email LIKE ".$db->quote('%'.$query.'%', 'STRING').';';
				$sql = 'SELECT * FROM `hrm_employees` '
						."LEFT JOIN hrm_agreements ON hrm_employees.employee_id=hrm_agreements.employee_id WHERE DATE(hrm_agreements.start_date) <= NOW() AND DATE(hrm_agreements.effective_date) >= NOW() - INTERVAL 1 DAY"
						." AND hrm_employees.email LIKE ".$db->quote('%'.$query.'%', 'STRING').''
								." ORDER BY ".$table.".".$sort." ".$dir." LIMIT "
		...
		$sql_count = "SELECT * FROM `hrm_employees` LEFT JOIN hrm_agreements ON hrm_employees.employee_id=hrm_agreements.employee_id WHERE DATE(hrm_agreements.start_date) <= NOW() AND DATE(hrm_agreements.effective_date) >= NOW() - INTERVAL 1 DAY;";
		$sql = 'SELECT * FROM `hrm_employees` '
		."LEFT JOIN hrm_agreements ON hrm_employees.employee_id=hrm_agreements.employee_id WHERE DATE(hrm_agreements.start_date) <= NOW() AND DATE(hrm_agreements.effective_date) >= NOW() - INTERVAL 1 DAY ORDER BY ".$table.".".$sort." ".$dir." LIMIT " 
		. $db->quote($start, 'INTEGER') . ', '. $db->quote($end, 'INTEGER') .';';
		...
		//Core::encrypt_data($config->mcrypt, $plain_text);
		
		//$plain_text = $items['sotu'];
		
		//$encrypt = $core->encrypt_data($config->mcrypt.$config->salt, $plain_text);
		//$decrypt = $core->decrypt_data($config->mcrypt.$config->salt, $encrypt);
	...
	public function createnewAction()
	{
	    ...
	    if (file_exists($target)) {
		    ...
		    $success = array('success' => true, 'msg' => $translate->_("Careers_New_Career_Created"));
		    
	    } else {
	        //echo "The file $filename does not exist";
	    	$success = array('success' => false, 'msg' => $translate->_("Careers_New_Career_Failed"));
	    }
    ...
    public function createnewqualificationAction()
	{
	    ...
	    if (file_exists($target)) {
		    //echo "The file $filename exists";
		    $success = array('success' => true, 'msg' => $translate->_("Careers_New_Qualification_Created"), 'employee_id' => $id);
		    
	    } else {
	        //echo "The file $filename does not exist";
	    	$success = array('success' => false, 'msg' => $translate->_("Careers_New_Qualification_Failed"), 'employee_id' => $id);
	    }
	...
	public function createnewtaxcardAction()
	{
		/** Object variable. Example use: $logger->err("Some error"); */
		$logger = Zend_Registry::get('LOGGER');
		/** Object variable. Example use: $something = $config->database; */
		...
		//$active = (string) $request->getPost('active');
		$date_come_to_effective = (string) date("Y-m-d",strtotime($request->getPost('date_come_to_effective')));
		$id = (integer) $request->getPost('employee_id');
		$sotu = (string) $db->fetchone("SELECT sotu FROM hrm_employees WHERE employee_id = ".$db->quote($id, 'INTEGER').";");
	
		$next_id = (integer) $db->fetchone("SELECT Auto_increment FROM information_schema.tables WHERE TABLE_SCHEMA = '".$config->database->params->dbname."' AND TABLE_NAME='hrm_taxcards';");
		...
		$file = (string) $insertID."_".md5($firstname_lastname).md5($sotu).'.pdf';
	
		//INSERT INTO `hrm-mml`.`hrm_employees` (`employee_id`, `fullname`, `sotu`, `address`, `zip`, `city`, `phone`, `email`, `taxnumber`, `cv_file`) VALUES (NULL, '', '', '', '', '', '', '', '', '');
	
		//$sql = "INSERT INTO `ostoreskontra` (`ostoreskontra_id`, `toimittaja_id`, `mml_viite`, `pankkimaksu_viite`, `laskun_pvm`, `laskunera_pvm`, `toimitusehto`, `laskun_summa_veroton`, `laskun_summa_verollinen`, `summa_debet`, `laskun_status`, `laskun_nro`, `created_by`, `seuraava_kasittelija_id`, `old_filename`, `veron_osuus`) VALUES (NULL, '".$db->quote($id, 'INTEGER')."', ".$db->quote($mml_viite, 'STRING').", '".$db->quote($pankkimaksu_viite, 'INTEGER')."', ".$db->quote($date, 'STRING').", ".$db->quote($duedate, 'STRING').", ".$db->quote($toimitusehto, 'STRING').", '".$db->quote($laskun_summa_varoton, 'DOUBLE')."', '".$db->quote($laskun_summa_verollinen, 'DOUBLE')."', '".$db->quote($laskun_summa_verollinen, 'DOUBLE')."', '1', '".$db->quote($number, 'INTEGER')."', '".$db->quote($userId, 'INTEGER')."', '".$db->quote($userId, 'INTEGER')."', ".$db->quote($file, 'STRING').", ".$db->quote($tax, 'STRING').");";
	
		$sql = "INSERT INTO `hrm_taxcards` (`taxcard_id`, `file_name`, `date_come_to_effective`, `employee_id`) VALUES (NULL, ".$db->quote($file, 'STRING').", ".$db->quote($date_come_to_effective, 'STRING').", ".$db->quote($id, 'INTEGER').");";
		//INSERT INTO `hrm_taxcards` (`taxcard_id`, `file_name`, `date_come_to_effective`, `employee_id`) VALUES (NULL, ".$db->quote($file, 'STRING').", ".$db->quote($date_come_to_effective, 'STRING').", ".$db->quote($id, 'INTEGER').");
		$db->query($sql);
	
		if(isset($_FILES['tcpath'])){
	
			$target = APPLICATION_PATH."/uploads/taxcards/".basename($file) ;
			//print_r($_FILES);
	
			if(move_uploaded_file($_FILES['tcpath']['tmp_name'],$target));
			//echo "OK!";//$chmod o+rw galleries
		}
	
		if (file_exists($target)) {
			//echo "The file $filename exists";
			$success = array('success' => true, 'msg' => $translate->_("Careers_New_Taxcard_Created"), 'employee_id' => $id);
	
		} else {
			//echo "The file $filename does not exist";
			$success = array('success' => false, 'msg' => $translate->_("Careers_New_Taxcard_Failed"), 'employee_id' => $id);
		}
	...
	public function downloadtaxcardAction()
	{
		/** Object variable. Example use: $logger->err("Some error"); */
		$logger = Zend_Registry::get('LOGGER');
		/** Object variable. Example use: $something = $config->database; */
		...
		try {
			...
			$file = (string) $db->fetchone("SELECT file_name FROM hrm_taxcards WHERE taxcard_id = ".$db->quote($id, 'INTEGER').";");
			$date_come_to_effective	 = (string) $db->fetchone("SELECT date_come_to_effective FROM hrm_taxcards WHERE taxcard_id = ".$db->quote($id, 'INTEGER').";");
			$employee_id = (integer) $db->fetchone("SELECT employee_id FROM hrm_taxcards WHERE taxcard_id = ".$db->quote($id, 'INTEGER').";");
			//$education_id = (integer) $db->fetchone("SELECT qualification_name FROM hrm_qualifications WHERE qualification_id = ".$db->quote($id, 'INTEGER').";");
			$fullname = (string) str_replace(' ', '_', $db->fetchone("SELECT CONCAT(lastname, ' ', firstname) FROM hrm_employees WHERE employee_id = ".$db->quote($employee_id, 'INTEGER').";"));
			//$education_name = (string) str_replace(' ', '_', $db->fetchone("SELECT CONCAT(education_name, ' ', education_type) FROM hrm_education_names WHERE education_id = ".$db->quote($education_id, 'INTEGER').";"));
			...
			$content = file_get_contents(APPLICATION_PATH."/uploads/taxcards/".$file);
	
			header('Content-Type: application/pdf');
			header("Content-Length: " . strlen($content) );
			header('Content-Disposition: attachment; filename='.$date_come_to_effective."_".$fullname.'.pdf');
    ...
    public function qualificationsAction()
	{
		...
		.' WHERE hrm_qualifications.employee_id = '.$db->quote($id, 'INTEGER').' ORDER BY qualification_id ASC;';
	...
	public function taxcardsAction()
	{
		/** Object variable. Example use: $logger->err("Some error"); */
		$logger = Zend_Registry::get('LOGGER');
		/** Object variable. Example use: $something = $config->database; */
		...
		$sql_count = 'SELECT * FROM `hrm_taxcards` WHERE employee_id = '.$db->quote($id, 'INTEGER').';';
		$sql = "SELECT `hrm_taxcards`.`taxcard_id`, `hrm_taxcards`.`date_come_to_effective`, `hrm_taxcards`.`employee_id`, CONCAT(hrm_employees.firstname, ' ', hrm_employees.lastname) as fullname  FROM `hrm_taxcards` LEFT JOIN hrm_employees ON hrm_employees.employee_id=hrm_taxcards.employee_id WHERE hrm_taxcards.employee_id = ".$db->quote($id, 'INTEGER')." ORDER BY hrm_taxcards.taxcard_id ASC LIMIT "
					. $db->quote($start, 'INTEGER') . ', '. $db->quote($end, 'INTEGER') .';';
    ...
    public function movecrAction()
	{
		...
		{
		   $id = (integer) $row_id;
		   $data = array('jobseeker'  => "true");
           $where = array("{$db->quoteIdentifier('employee_id')} = ?" => $id);
           $db->update('hrm_employees', $data, $where);
		   
		}
		
	    $msg = $translate->_("Career_Employee_Moved");
    ...
    public function deleteqaAction()
	{
	    ...
	    $msg = $translate->_("Career_Qualification_Deleted");
	...
	public function deletetcAction()
	{
		...
		foreach($selectedRows as $row_id)
		{
			...
			$sql = "DELETE FROM `hrm_taxcards` WHERE `taxcard_id` = ?;";
	
			$attachment = (string) $db->fetchone("SELECT file_name FROM hrm_taxcards WHERE taxcard_id = ".$db->quote($id, 'INTEGER').";");
			$employee_id = (integer) $db->fetchone("SELECT employee_id FROM hrm_taxcards WHERE taxcard_id = ".$db->quote($id, 'INTEGER').";");
			 
			$file_path = APPLICATION_PATH."/uploads/taxcards/".$attachment;
			unlink($file_path);
		...
		$msg = $translate->_("Career_Taxcard_Deleted");
    ...
    public function replaceAction()
	{
		...
		$msg = $translate->_("Careers_Replaced");;
	...
	public function replacetcAction()
	{
		...
		$id = (integer) $request->getParam('replace_tc_id');
		...
		$msg = $translate->_("Careers_Replaced_Tc");;
