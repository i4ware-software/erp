 ...
 Ext.onReady(function() {
	 ...
	 function years(val) {               
         ...
         return '<span style="color:#008800;">' + val + '<?= $this->years ?></span>';
            ...
            var store = new Ext.data.Store({
                url: '/zf/public/agreements/json/index',
                reader: new Ext.data.JsonReader({root: 'agreements',
                    totalProperty: 'totalCount',id: 'agreement_id'}, 
                        [{name: 'agreement_id',type: 'int'},
                         ...
                         {name: 'trial_end_date',type: 'date', dateFormat:'Y-m-d'},
    ...
    var storeType = new Ext.data.Store ({
      ...
      var storeWorktime = new Ext.data.Store ({
        ...
        /*var storeSalaryPerformanceForm = new Ext.data.Store ({
      		proxy: new Ext.data.HttpProxy ({ 
      			url: '/zf/public/agreements/json/performancetermsform',
      			scope: this
      		})
      		, baseParams: {
      			task: "performacetermsform"
      		},
      		sortInfo:{field: 'KeyField', direction: "ASC"}
      		, reader: new Ext.data.JsonReader ({
      			root: 'performancetermsform_root'
      			, id: 'KeyField'
      			, fields: [
      				{name: 'KeyField'}
      				, {name: 'DisplayField'}
        ...
        storeSalaryPerformanceForm.loadData;
        storeSalaryPerformanceForm.load();*/
        ...
        storeTes.load();
         ...
         // create the Grid
                ...
                tbar: [
		                ...
		                },
		                {
	                        ...
	                        handler: function () {                
                                ...
                                window.location = '/zf/public/agreements/json/activeagreementsxls';
                                
                            }
	                },
	                {
                        id: 'export-emails-grid',
                    	text: '<?= $this->export_emails_excel ?>',
                        tooltip: '<?= $this->export_emails_excel_tooltip ?>',
                        iconCls: 'refresh-icon',
                        disabled: false,
                        handler: function () {                
                                             
                            window.location = '/zf/public/agreements/json/listofemailsexcel';
               ...
               // Collapse combo when its element is clicked on
            	   listeners : {
            	      ...
            	      if(newValue)
            	      {
            	    	 ...
            	    	 var starttime = Ext.getCmp('start_date').getValue();	
				    	 //var endtime = value;
				    	 //var delta = Date.parse(endtime) - Date.parse(starttime);
				    	 //alert(delta / 86400 / 3600);
   	                     //var days = parseInt(parseInt(delta / 86400000) / 2);
   	                     
   	                     var trialDate = new Date(starttime).add(Date.MONTH, 4);
   	                     
   	                     Ext.getCmp('form_trial_end_date').setValue(trialDate);
           ...
           var RadioCreateUser = new Ext.form.Checkbox({
              	  id:'create_user',
              	  //xtype: 'radio',
                  fieldLabel: '',
                  labelSeparator: '',
                  boxLabel: '<?= $this->create_user ?>',
                  name: 'create_user',
                  inputValue: 'yes',
                  // Collapse combo when its element is clicked on
               	   listeners : {
                ...
                }),*/
            	{
                    ...
                    listeners: {
					    change: function (combo, value) {
					    	 //alert(value);
					    	 var starttime = value;	
					    	 var endtime = Ext.getCmp('effective_date').getValue();
					    	 var delta = Date.parse(endtime) - Date.parse(starttime);
					    	 //alert(delta / 86400 / 3600);
        	                 var days = parseInt(parseInt(delta / 86400000) / 2);
        	                 var months;
        	                 ...
        	                 //alert(days);
        	                 if (days==0) {
        	                	 trialDate = new Date(value).add(Date.MONTH, 4);
        	                 } else {
        	                	 
        	                	 //alert();
        	                	 months = DateDiff.inMonths(starttime, endtime);
        	                	 if (months<=7) {
        	                		 trialDate = new Date(value).add(Date.DAY, days);
        	                	 } else {
        	                		 trialDate = new Date(value).add(Date.MONTH, 4);
        	                	 }
					    ...
					    change: function (combo, value) {
					    	 ...
					    	 //alert(delta / 86400 / 3600);
       	                     ...
       	                     if (isNaN(days)) {
     	                    	...
     	                    	if (months<=7) {
       	                		    trialDate = new Date(starttime).add(Date.DAY, days);
       	                	      } else {
       	                		    trialDate = new Date(starttime).add(Date.MONTH, 4);
       	                	    }
                    ...
                    id: 'form_trial_end_date',
                	fieldLabel : '<?= $this->trial_end_date ?>',
                    name : 'trial_end_date',
                    allowBlank : false,
                    xtype: 'datefield',
                    format: 'd.m.Y'
                    ...
                    buttonText: 'Browse...'
                        ...
                        items: [{
                            ...
                            items: [RadioSalaryPeriodI, RadioSalaryPeriodII, RadioSalaryPeriodIII, RadioSalaryPeriodIV]
                                ...
                                vertical: true,
                                columns: 2,
                                items: [RadioSalaryTime, RadioSalaryPerformance, RadioSalaryOther, RadioSalaryComission]
                            },{
                            	xtype: "textfield",
                            	fieldLabel: "",
                            	id: "from_date",
