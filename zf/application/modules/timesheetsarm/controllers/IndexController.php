...
class Timesheetsarm_IndexController extends Zend_Controller_Action
{
	...
	public function timesheetAction()
    {
        ...
        if ($type=="salary") {
        	...
        	$sql = 'SELECT * FROM `hrm_timesheet_hours_dates` '
        			...
        			.' AND user_id = '
        					...
        					//." AND action_date != '0000-00-00'"
        					//." AND action_date != '1970-01-01'"
        					. " AND project_id != 0"		
        					.' ORDER BY order_id;';
        	
        	$this->view->type = "salary";
		...
		$this->view->memo_missing = $translate->_("Timesheetsarm_Memo_Missing");
		$this->view->hour_status_missing = $translate->_("Timesheetsarm_Hour_Status_Incorrect");
		$this->view->hour_status_missing_not_can = $translate->_("Timesheetsarm_Hour_Status_Incorrect_Not_Can");
		$this->view->hour_status_missing_not_can_all = $translate->_("Timesheetsarm_Hour_Status_Incorrect_Not_Can_All");
		...
		//$this->view->permanent = (string) str_replace(array('true','false'), array($translate->_("Timesheetsarm_True"),$translate->_("Timesheetsarm_False")), $db->fetchone($sql_permanent));
		...
		//print_r($fetch_array);
		 ...
		 $timesheet_status = $db->fetchOne("SELECT `status` FROM `hrm_timesheets_index` WHERE timesheet_id = ".$db->quote($timesheet_id, 'INTEGER').";");
		 
		 if ($timesheet_status == 6 || $timesheet_status == 7) {
		 
		 $this->view->status = (string) "true";
		 
		 } else {
		 
		 $this->view->status = (string) "false";
			   ...
			   $this->view->assign($key_id.'_'.$i, $value_by_id);
				...
				while($row = $stmt_hour_status_id->fetch())
				{
					...
					if ($value_by_id==$fetch_id) {
						...
						//selected
						if ($timesheet_status == 6 || $timesheet_status == 7) {
						    $fetch_html .= "<option value=\"".$fetch_id."\" selected disabled>".$fetch_value."</option>";
						} else {
						...
						if ($timesheet_status == 6 || $timesheet_status == 7) {
							...
							$fetch_html .= "<option value=\"".$fetch_id."\" disabled>".$fetch_value."</option>";
        ...
        if ($_GET['posted'] == "true") {
        $this->_redirect('https://'.$config->webhost.'/zf/public/timesheet/index/timesheet?timesheet_id='.$timesheet_id);
        }*/
    ...
    public function returnAction()
    {
    	...
    	$sql = "INSERT INTO `hrm_timesheet_history` (`history_id`, `timesheet_id`, `datetime_created`, `user_id`, `description`) VALUES (NULL, ".$db->quote($timesheet_id, 'INTEGER').", '".$date_string."', ".$db->quote($userId, 'INTEGER').", '".$translate->_("Timesheetsarm_Return_To_Employee")."');";
    	$db->query($sql);
    	...
    	$data = array("next_user" => $idUser);
    	$where = array("{$db->quoteIdentifier('timesheet_id')} = ?" => $timesheet_id, "{$db->quoteIdentifier('project_id')} != ?" => 0, "{$db->quoteIdentifier('hour_status_id')} != ?" => 2);
    	$db->update('hrm_timesheet_hours_dates', $data, $where);
    	...
    	$CustomerContacPersonEmail = "";
    	$CustomerContacPersonCompany = "";
    	...
    	foreach ($poject_ids_array as $key => $value) {
    		if ($nr<$n) {
    			...
    			$user_Id_for_customer = (integer) $db->fetchone("SELECT customer_id FROM hrm_workplaces WHERE workplace_id = ".$db->quote($value, 'INTEGER').";");
    	...
    	$raw = $db->fetchOne("SELECT bodytext FROM erp_emails WHERE email_id = 9;");
    	$html = $db->fetchOne("SELECT bodyhtml FROM erp_emails WHERE email_id = 9;");
    	
    	$subject = utf8_decode($db->fetchOne("SELECT subject FROM erp_emails WHERE email_id = 9;"));
    	//$html_body_text = $translate->_("Timesheet_Email_Body_Text_3")
    	//.'<br /><br />'.$translate->_("Timesheet_Email_Link_Text_1").'<br /><a href="http://'.$config->webhost.'/zf/public/">http://'.$config->webhost.'/zf/public/</a>!';
    	//$html_raw_text = $translate->_("Timesheet_Email_Body_Text_3")." ".$translate->_("Timesheet_Email_Link_Text_1")." "
    	//.' http://'.$config->webhost.'/zf/public/';
    	$search = array("{fullname}", "{erp_link}", "{company_name}", "{switch_phonenumber}", "{company_email}", "{fullname_employee}", "{workplace}", "{startdate}", "{enddate}", "{customer_person_company}", "{customer_person_name}", "{customer_person_email}");
    	$replace = array($ToEmailName, "http://".$config->webhost."/", $company_name, $switch_phonenumber, $company_email, $person, $workplaces, $min_date, $max_date, $CustomerContacPersonCompany, $CustomerContactPersonName, $CustomerContacPersonEmail);
    	 
    	$raw = str_replace($search,$replace,$raw);
    	 
    	$search = array("{fullname}", "{erp_link}", "{company_name}", "{switch_phonenumber}", "{company_email}", "{fullname_employee}", "{workplace}", "{startdate}", "{enddate}", "{customer_person_company}", "{customer_person_name}", "{customer_person_email}");
    	$replace = array($ToEmailName, '<a href="http://'.$config->webhost.'/">'.$translate->_("Timesheet_HTML_Link_Text_2").'</a>', $company_name, $switch_phonenumber, $company_email, $person, $workplaces, $min_date, $max_date, $CustomerContacPersonCompany, $CustomerContactPersonName, $CustomerContacPersonEmail);
    	...
    	$this->_redirect('http://'.$config->webhost.'/zf/public/timesheetsarm/index/index');
    	
    }
    
    public function senttocrmAction()
    {
    	...
    	$sql = "INSERT INTO `hrm_timesheet_history` (`history_id`, `timesheet_id`, `datetime_created`, `user_id`, `description`) VALUES (NULL, ".$db->quote($timesheet_id, 'INTEGER').", '".$date_string."', ".$db->quote($userId, 'INTEGER').", '".$translate->_("Timesheetsarm_Sent_To_Hrm")."');";
    	$db->query($sql);
    	
    	//pitŠŠ tarkistaa, ettŠ onko kaikki tyštunnit hyvŠksytty vai ei.
    	...
    	$raw = $db->fetchOne("SELECT bodytext FROM erp_emails WHERE email_id = 8;");
    	$html = $db->fetchOne("SELECT bodyhtml FROM erp_emails WHERE email_id = 8;");
    	 
    	$subject = utf8_decode($db->fetchOne("SELECT subject FROM erp_emails WHERE email_id = 8;"));
    	
    	//$html_body_text = $translate->_("Timesheet_Email_Body_Text_5")
    	//.'<br /><br />'.$translate->_("Timesheet_Email_Link_Text_1").'<br /><a href="http://'.$config->webhost.'/zf/public/">http://'.$config->webhost.'/zf/public/</a>!';
    	//$html_raw_text = $translate->_("Timesheet_Email_Body_Text_5")." ".$translate->_("Timesheet_Email_Link_Text_1")
    	//.' http://'.$config->webhost.'/zf/public/';
