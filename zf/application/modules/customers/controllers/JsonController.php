...
class Customers_JsonController extends Zend_Controller_Action
{
   ...
   public function indexAction()
	{
		...
		$sql_count = "SELECT * FROM `hrm_customers` WHERE customer_name LIKE ".$db->quote('%'.$query.'%', 'STRING').';';
		$sql = 'SELECT * FROM `hrm_customers` '
		."WHERE customer_name LIKE ".$db->quote('%'.$query.'%', 'STRING').""
		." ORDER BY ".$sort." ".$dir." LIMIT " 
		. $db->quote($start, 'INTEGER') . ', '. $db->quote($end, 'INTEGER') .';';
			
		} else {
		
		$sql_count = "SELECT * FROM `hrm_customers`;";
		$sql = 'SELECT * FROM `hrm_customers`'
		." ORDER BY ".$sort." ".$dir." LIMIT " 
		. $db->quote($start, 'INTEGER') . ', '. $db->quote($end, 'INTEGER') .';';
    ...
    public function grideditAction()
	{
		...
		*/
		...
		$value = (string) strip_tags(stripslashes($request->getPost('value')));
            ...
            $where = array("{$db->quoteIdentifier('customer_id')} = ?" => $id);
            $db->update('hrm_customers', $data, $where);
    ...
    public function createnewAction()
	{
		...
		$customer_name = (string) $request->getPost('customer_name');
		...
		$sql = "INSERT INTO `hrm_customers` (`customer_id` , `customer_name`) VALUES (NULL , ". $db->quote($customer_name, 'STRING').");";
		
		$db->query($sql);
		
        $success = array('success' => true, 'msg' => $translate->_("Customers_New_Customer_Created"));
    ...
    public function deleteAction()
	{
		...
		foreach($selectedRows as $row_id)
		{
		   ...
		   $sql = "DELETE FROM `hrm_customers` WHERE `customer_id` = ?;";
	    ...
	    $msg = $translate->_("Custoemrs_Customer_Deleted");
