...
function eraseCookie(name) {
 ...
 Ext.onReady(function() {
            ...
            var store = new Ext.data.Store({
                    url: '/zf/public/projects/json/index',
                    reader: new Ext.data.JsonReader({root: 'projects',
                        totalProperty: 'totalCount',id: 'projekti_id'}, 
                            [{name: 'projekti_id', type: 'int'},
                            {name: 'nimi_nro', type: 'string'},
                            {name: 'kustannuspaikka_id', type: 'string'}]),
                            baseParams: { "limit":50 },
                            sortInfo:{field: 'projekti_id', direction: "DESC"},
                            remoteSort: true
                        });
                        
             store.load({params: { "start":0, "limit":50, "query":"" }});
             
             var storeProfitcenterId = new Ext.data.Store ({
 				proxy: new Ext.data.HttpProxy ({ 
 					url: '/zf/public/projects/json/profitcenter',
 					scope: this
 				})
 				, baseParams: {
 					task: "profitcenter"
         ...
         storeProfitcenterId.loadData;
         storeProfitcenterId.load();
            
            function saveEdit (Grid_Event) {
				Ext.Ajax.request({
					waitMsg: '<?= $this->sending ?>'
					, url: '/zf/public/projects/json/edit'
					, params: { 
						task: "edit"
						, key: 'projekti_id' 
						, keyID: Grid_Event.record.data.projekti_id
			...
			function handleDelete() {
				...
				Ext.Ajax.request({
					waitMsg: '<?= $this->sending ?>'
					, url: '/zf/public/projects/json/delete'
					, params: { 
						task: "delete"
            ...
            var newform = new Ext.FormPanel( {
                id : Ext.id(),
                labelWidth : 120,
                url : "/zf/public/projects/json/createnew",
                frame : false,
                                ...
                                buttons : [
                                        {
                                            text : '<?= $this->submit ?>',
                                            handler : function() {
                                                var url = "/zf/public/projects/json/createnew";
            ...
            var replaceform = new Ext.FormPanel( {
                id : Ext.id(),
                labelWidth : 120,
                url : "/zf/public/projects/json/import",
                frame : false,
                bodyStyle : 'padding:5px 5px 0 0',
                width : 460,
                border : false,
                                ...
                                buttons : [
                                        {
                                            text : '<?= $this->submit ?>',
                                            handler : function() {
                                                var url = "/zf/public/projects/json/import";
    ...
    var grid = new Ext.grid.EditorGridPanel({
        ...
        cm: new Ext.grid.ColumnModel({
            ...
            },
            {
                ...
                sortable : true,
                locked:true, 
                dataIndex: 'projekti_nimi',
                editor: new fm.TextField({
					allowBlank: false
				})
            },
            {
                header   : '<?= $this->nimi_nro ?>', 
                width    : 140, 
                sortable : true,
                locked:true, 
                dataIndex: 'nimi_nro',
                ...
                id       :'profitcenter',
                header   : '<?= $this->profitcenter ?>', 
                width    : 200, 
                sortable : false,
                locked:false,
                dataIndex: 'kustannuspaikka_id',
                editor: new Ext.form.ComboBox({									    
					store: storeProfitcenterId,
				...
				, renderer: function(data) {
					record = storeProfitcenterId.getById(data);
					if(record) {
        ...
        tbar: [
                ...
                handler: function () {
                    ...
                    iconCls: 'refresh-icon',
                    handler: function () {
                    	window.location = '/zf/public/projects/json/export';
                }},
                {
                    text: '<?= $this->import ?>',
