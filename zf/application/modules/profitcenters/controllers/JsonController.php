...
class Profitcenters_JsonController extends Zend_Controller_Action
{
   ...
   public function indexAction()
	{
        ...
        $table = "ostoreskontra_kustannuspaikkat";
        
        if ($fields == "kustannuspaikka_id") {
		
		$sql_count = 'SELECT kustannuspaikka_id FROM ' . $table
		.' WHERE '. $table .'.kustannuspaikka_id LIKE '.$db->quote('%'.$query.'%', 'STRING')
		. ';';
        ...
        } else if ($fields == "kustannuspaikka_nimi") {
            
            $sql_count = 'SELECT kustannuspaikka_id FROM ' . $table
            .' WHERE '. $table .'.kustannuspaikka_nimi LIKE '.$db->quote('%'.$query.'%', 'STRING')
            . ';';
            $sql = 'SELECT * FROM ' . $table
            .' WHERE '. $table .'.kustannuspaikka_nimi LIKE '.$db->quote('%'.$query.'%', 'STRING').' '
                ."ORDER BY ".$table.".".$sort." ".$dir." LIMIT "
	...
	public function createnewAction()
	{
	    ...
	    $kustannuspaikka_id = (integer) $db->fetchone('SELECT MAX(kustannuspaikka_id) FROM ostoreskontra_kustannuspaikkat;') + 1;
	    $kustannuspaikka_nimi = (string) $request->getPost('kustannuspaikka_nimi');
	    
	    $sql = "INSERT INTO `ostoreskontra_kustannuspaikkat` (`kustannuspaikka_id`, `kustannuspaikka_nimi`) VALUES (".$db->quote($kustannuspaikka_id, 'INTEGER').", ".$db->quote($kustannuspaikka_nimi, 'STRING').")";
	 ...
	 public function editAction()
	 {
	     ...
	     $where = array("{$db->quoteIdentifier('kustannuspaikka_id')} = ?" => $id);
	     $db->update('ostoreskontra_kustannuspaikkat', $data, $where);
	 ...
	 public function deleteAction()
	 {
	     ...
	     {
	         $id = (integer) $row_id;
	         $sql = "DELETE FROM `ostoreskontra_kustannuspaikkat` WHERE `ostoreskontra_kustannuspaikkat`.`kustannuspaikka_id` = ?;";
	         if ($db->query($sql,$id)) {
	             $success = array('success' => true);
	         } else {
	             $success = array('success' => false);
	 ...
	 public function exportAction()
	 {
	     ...
	     $sql = "SELECT * FROM ostoreskontra_kustannuspaikkat;";
	     ...
	     while($row = $stmt->fetch()) {
	         ...
	         $objPHPExcel->setActiveSheetIndex(0)
	         ->setCellValue('A'.$i, $row['kustannuspaikka_id'])
	         ->setCellValue('B'.$i, $row['kustannuspaikka_nimi']);
	 
	         $i++;
	 
	     }
	 
	     $file = str_replace(" ", "_", $config->portal)."_kustannuspaikat.xls";
	 ...
	 public function importAction()
	 {
	     ...
	     $file = (string) "import_profitcenters.xls";
	     ...
	     $sql = "TRUNCATE TABLE `ostoreskontra_kustannuspaikkat`;";
	 
	     $db->query($sql);
	 
	     $sql = "INSERT INTO `ostoreskontra_kustannuspaikkat` (`kustannuspaikka_id`, `kustannuspaikka_nimi`) VALUES ";
